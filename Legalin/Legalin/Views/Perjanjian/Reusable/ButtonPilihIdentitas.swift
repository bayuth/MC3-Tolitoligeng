...
struct ButtonPilihIdentitas: View {
    ...
    var body: some View {
		Button(action: {
			...
			Alert(title: Text("Kambing Soon!"), message: Text("Fitur ini masih dalam tahap development, maafkan developer kami yang kerjanya lambat :)"), dismissButton: .destructive(Text("Ok").fontWeight(.black)))
		}).padding(.bottom,13)
    }
}
